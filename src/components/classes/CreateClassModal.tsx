...
const CreateClassModal: React.FC<CreateClassModalProps> = ({ schoolId, onClassCreated }) => {
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="mt-3 grid grid-cols-3 gap-3">
          <div className="relative">
            ...
            >
              <option value="" disabled className="bg-[#6F6D95] text-white">Days</option>
              {daysOfWeek.map((day) => (
                <option key={day.value} value={day.value} className="bg-[#6F6D95] text-white">
                  {day.label}
                </option>
              ))}
            </select>
            <button
              ...
              onClick={() => {
                const el = document.getElementById('day_of_week') as HTMLSelectElement
                el?.focus()
              }}
